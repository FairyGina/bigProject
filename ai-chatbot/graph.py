...
def generate_recipe_node(state):
    ...
    trend_rule = (
        "- 트렌드 반영이 '예'이고 국가가 '한국'인 경우:\n"
        "  한국의 최신 식문화 트렌드를 참고하여 레시피를 생성한다.\n"
        "- 트렌드 반영이 '예'이고 국가가 (미국/중국/일본/베트남/독일)인 경우:\n"
        "  SerpAPI 트렌드 요약 + 수요예측 상위 품목을 참고하여 레시피를 생성한다.\n"
        "- 트렌드 반영이 '아니오'이거나 해당 국가가 아니면:\n"
        "  트렌드 언급 없이 일반 레시피로 생성한다."
    )

    trend_query_prompt = f"""
...
"""
    ...
    if forecast_items:
        ...
        forecast_block = (
            ...
            f"- 레시피 사용 후보(최종 0~2개): {FORECAST_SELECTED_TOKEN}\n"
            "- 참고: 우선 고려 후보 중 0~2개만 선택해 사용 (어울리지 않다고 판단되면 미사용)\n"
        )
    ...
    [요리 상식 규칙]
    - 모든 레시피에는 다음을 포함해야 한다:
    - 주재료 1개 이상
    - 지방(식용유, 버터 등) 1개 이상
    - 향미 재료(마늘, 양파, 허브 등) 1개 이상
    - 조미 재료(소금, 소스, 장류 등) 1개 이상
    - 조미료만으로 구성된 레시피는 생성하지 않는다.
    - 명확한 맛의 방향(짠맛, 단맛, 감칠맛 등)을 유지한다.
    - 근거 없는 충돌 조합은 피한다.
    - 강한 산성 재료와 유제품은 고온 조리 시 주의한다.
    - 메뉴 정체성을 깨는 조합은 피한다.
    - 향미 재료는 지방에서 먼저 조리한다.
    - 단단한 재료를 먼저, 부드러운 재료를 나중에 조리한다.
    - 조미는 단계적으로 추가한다.
    - 조리 순서와 과정은 물리적으로 가능해야 한다.
    - 하나의 명확한 요리 문화(한식, 양식 등)를 기준으로 한다.
    - 이질적인 재료를 섞을 경우, 지배적인 맛의 베이스를 명확히 한다.
    - 반드시 익혀야 하는 재료는 충분히 조리한다.
    - 안전하지 않거나 비상식적인 섭취 방식은 사용하지 않는다.
    ...
    - 고강도 맛 조합은 상호 억제를 일으키므로 강한 맛+강한 맛 조합을 피한다.
      강한 맛 + 약한 보조 맛만 허용한다.
    - 맛의 충돌이 발생한다고 판단되면 충돌을 일으키는 재료 중 하나는 제외하거나
      더 적합한 재료로 대체한다.
...
"""
    ...
    if trend_enabled and country in ({"한국"} | FORECAST_COUNTRIES):
        state["trend_query_prompt"] = trend_query_prompt.strip()
        state["trend_summary_prompt"] = trend_summary_prompt.strip()
    else:
        state["trend_query_prompt"] = None
        state["trend_summary_prompt"] = None
    state["recipe"] = None
